...
class MomentScreenNotifier extends ScreenNotifier {
    ...
    if(Platform.isIOS){
      ...
      if (index == 1) {
        ...
        if (result != null) {
          print(result.path);
          showChangeProfileDialog(
              imageUrl: result.path, isCircle: true,
              context: context,
              onConfirm: () async {
  ...
  onReportTapped(id) {
    if(reasonController.text!="" || writeReviewKey.currentState!.validate()) {
      reportDeleteCubit.reportPost(
          ReportPostParam(description: reason, refId: id, refType: 2));
      reasonController.clear();
    }else{
      Fluttertoast.showToast(
          msg: Translation.current.errorTxt);
