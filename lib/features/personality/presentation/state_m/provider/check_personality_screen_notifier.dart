...
class CheckPersonalityScreenNotifier extends ScreenNotifier {
 ...
 /* void addAnswer(int id, PageController controller) {
  ...
  void addAnswerTwo(int  questionIndex ,int choiceIndex,PageController controller){
  ...
  void onCompleteTap() {
    if (canCompletNew()) {
      for (int i = 0; i < newAnswers.length; i++) {
        if(personalityQuestionListEntity.questions[newAnswers[i].questionIndex!].id != -1){
          AnswermModelRequest answerModelRequest = AnswermModelRequest(
              questionId: personalityQuestionListEntity.questions[newAnswers[i].questionIndex!].id,
              choiceId: personalityQuestionListEntity.questions[newAnswers[i].questionIndex!].choices[newAnswers[i].choiceIndex!].id);
      ...
      showErrorSnackBar(message: Translation.current.answer_all_personality_question);
    }
  }
  ...
  onGenderUpdated(UpdateProfileEntity profileEntity) async {
    ...
    UserSessionDataModel.updateProfileSP(
        profileEntity.name!, profileEntity.surname!, profileEntity.fullName!, UserSessionDataModel.birthday, getGenderQuestion(), profileEntity.imageUrl?? null, profileEntity.emailAddress!);
    final prefs = await SpUtil.getInstance();
    prefs.putBool(AppConstants.HAS_PERSONALITY_AVATAR, true);
    Nav.toAndRemoveAll(PersonalityDetailsScreen.routeName , arguments: PersonalityResultScreenParams(
      birthDay: UserSessionDataModel?.birthday != '' ? DateTime.parse(UserSessionDataModel.birthday) : null,
      gender: getGenderQuestion(),
    ));
  }
  ...
  getGenderQuestion(){
    for (int i = 0; i < newAnswers.length; i++) {
      if(personalityQuestionListEntity.questions[newAnswers[i].questionIndex!].id == -1){
        return personalityQuestionListEntity.questions[newAnswers[i].questionIndex!].choices[newAnswers[i].choiceIndex!].id;
      }
    }
  }
  ...
  generateUiData() {
    this.personalityQuestionListEntity.questions.forEach((element) {
      ...
      PersonalityQuestion tempQuestion = PersonalityQuestion(
        element.content,
        1,
        tempChoices,
          element.arContent, element.enContent
      );
...
class PersonalityQuestion {
  ...
  PersonalityQuestion(this.question,
      this.ratio,
      this.choices, this.arQuestion, this.enQuestion,);
}
