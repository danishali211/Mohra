...
class _NewsHomeScreenContentState extends State<NewsHomeScreenContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: sn.isSearch
          ? searchResult()
          : sn.isSearchBody ? BlocConsumer<NewsCubit, NewsState>(
        bloc: sn.newsSearchCubit,
        listener: (context, state) {
          if (state is CreationTimeNewsLoaded) {
            sn.onSearchNewsLoaded(state.newsOfCategory1Entity.result.items!);
          ...
          if(state is CreationTimeNewsLoaded){
              return Container(
                  ...
                  child: sn.isLoadingSearchNews
                      ? WaitingWidget() :PaginationWidget<NewsItemOfCategoryEntity>(
                    ...
                    ListView.separated(
                          ...
                          onTap: () {
                            Nav.to(
                              ...
                              arguments: SingleNewsParams(
                                  id: sn.NewsOfCreationTime[index].id!),
                            );
                          },
                          child: HealthNewsItem(
                            image: sn.NewsOfCreationTime[index].imageUrl!,
                            icon: sn.NewsOfCreationTime[index].category?.imageUrl??"",
                            title: sn.NewsOfCreationTime[index].arTitle!,
                            iconName: sn.NewsOfCreationTime[index].category?.imageUrl??"",
                            date: sn.NewsOfCreationTime[index].creationTime!,
                          ),
                        );
                      },
                      ...
                      itemCount: sn.NewsOfCreationTime.length,
                    ):
                    _buildEmptyWidget(),
                  ));
          }
                  ...
                  children: [
                      ...
                      child: BlocConsumer<NewsCubit, NewsState>(
                        ...
                        listener: (context, state) {
                          if (state is NewsCategoryLoaded) {
                            sn.onCategoriesLoaded(state.newsCategoryEntity.result.items);
                            // sn.getNewsHome();
                            sn.loading(false);
                          }
                          if(state is NewsErrorState) {
                        ...
                        builder: (context, state) {
                          return state.maybeMap(
                            ...
                            newsCategoryLoaded: (s) => Container(
                              ...
                              child: sn.isLoading
                                  ? WaitingWidget()
                                  : PaginationWidget<NewsCategoryItemEntity>(
                                scrollDirection: Axis.horizontal,
                                items: sn.allCategories,
                                getItems: sn.getCategoriesItems,
                                onDataFetched: sn.onCategoriesItemsFetched,
                                refreshController: sn.momentsRefreshController,
                                footer: ClassicFooter(
                                  loadingText: "",
                                  noDataText: Translation.of(context).noDataRefresher,
                                ...
                                child: ListView.builder(
                                  itemCount: sn.allCategories.length,
                                      ...
                                      onPress: () {
                                        Nav.to(
                                          NewsSubCategoryScreen
                                              .routeName,
                                          arguments:
                                          SingleCategoryParams(entity: sn.allCategories[index]),
                                        );
                                      },
                                      // name: sn.categories3[index].name!,
                                      name: sn.allCategories[index].arName!,
                                      image: sn
                                          .allCategories[
                                      index]
                                          .imageUrl!,
                    ...
                    for(int i=0;i<sn.allCategories.length;i++)
                      Padding(
                        ...
                        child: BlocConsumer<NewsCubit, NewsState>(
                          bloc: NewsCubit()..newsOfCategory1Home(NewsSingleCategoryParam(
                                        ...
                                        children: [
                                          Row(
                                            ...
                                            children: [
                                              ...
                                              Text(
                                                  state.newsOfCategory1Entity.result.items?.first.category?.arName??"",
                                                style: TextStyle(
                                                    fontSize: 40.sp,
                                                    color: AppColors.textLight2,
                                                    fontWeight: FontWeight.bold),
                                          ...
                                          ListView.separated(
                                              ...
                                              return InkWell(
                                                onTap: () {
                                                  Nav.to(
                                                    SingleNewsScreen.routeName,
                                                    arguments: SingleNewsParams(
                                                        id: state.newsOfCategory1Entity.result.items![index].id!),
                                                  );
                                                },
                                                child: HealthNewsItem(
                                                  image: state.newsOfCategory1Entity.result.items![index].imageUrl!,
                                                  icon: state.newsOfCategory1Entity.result.items?[index].category?.imageUrl??"",
                                                  title: state.newsOfCategory1Entity.result.items![index].arTitle!,
                                                  iconName: state.newsOfCategory1Entity.result.items?[index].category?.imageUrl??"",
                                                  date: state.newsOfCategory1Entity.result.items![index].creationTime!,
                                                ),
                                              );
                                            },
                                            separatorBuilder: (context, index) {
                                            ...
                                            itemCount: state.newsOfCategory1Entity.result.items!.length,
                                          ),
                                        ],
                                      ),
                                    ),
